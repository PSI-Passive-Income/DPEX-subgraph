...
import { updatePairDayData, updateTokenDayData, updateDPexDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = DPexFactory.load(FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  dpex.totalLiquidityBNB = dpex.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal)
  ...
  dpex.totalLiquidityBNB = dpex.totalLiquidityBNB.plus(trackedLiquidityBNB)
  dpex.totalLiquidityUSD = dpex.totalLiquidityBNB.times(bundle.bnbPrice)
...
export function handleMint(event: Mint): void {
  ...
  dpex.txCount = dpex.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let dpex = DPexFactory.load(FACTORY_ADDRESS)
  dpex.totalVolumeUSD = dpex.totalVolumeUSD.plus(trackedAmountUSD)
  dpex.totalVolumeBNB = dpex.totalVolumeBNB.plus(trackedAmountBNB)
  dpex.untrackedVolumeUSD = dpex.untrackedVolumeUSD.plus(derivedAmountUSD)
  dpex.txCount = dpex.txCount.plus(ONE_BI)
  ...
  let dpexDayData = updateDPexDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  dpexDayData.dailyVolumeUSD = dpexDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  dpexDayData.dailyVolumeBNB = dpexDayData.dailyVolumeBNB.plus(trackedAmountBNB)
  dpexDayData.dailyVolumeUntracked = dpexDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  dpexDayData.save()
